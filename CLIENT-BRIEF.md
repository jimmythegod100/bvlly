# Client Brief — BVLLY

Kit: **HTML business starter**, adapted into a clothing catalog.

---

## Business basics

| Field | Answer |
|-------|--------|
| **Business name** | BVLLY |
| **Short name** | BVLLY |
| **Tagline** | Streetwear drawn from the exotic bully. |
| **What they do** | Clothing brand — oversized hoodies, tees, cap. Narrative and essence from the exotic bully. Gothic Beware prints and embroidered marks. |
| **Years in business / credibility** | Draft / first drop. |
| **Service area** | Online. |

---

## Contact info

| Field | Answer |
|-------|--------|
| **Email** | hello@bvlly.com (placeholder until a real inbox is confirmed) |
| **Phone** | TBD |
| **Address** | Online only |
| **Business hours** | Drop inquiries within two business days |
| **Social links** | Instagram TBD |

---

## Site goals

What should the website **make visitors do**?

- [x] Browse a clothing catalog
- [x] Learn the fit story
- [x] Join a drop waitlist / email the label
- [ ] Checkout (not in this draft)

---

## Pages needed

| Page | Include? | Notes |
|------|----------|-------|
| Home | Yes | Hero, featured, collections, fit notes |
| Shop / catalog | Yes | Filters by collection |
| Product | Yes | Size/color, notify me |
| Collections | Yes | Beware, The Mark, Accessories |
| About | Yes | Brand story |
| Contact | Yes | Waitlist form |
| Privacy | Yes | FormSubmit note |

---

## Brand & design

| Field | Answer |
|-------|--------|
| **Ground** | Near-black `#070707` |
| **Type / chrome** | Silver `#cfc6b8` |
| **Photo well** | Cream `#ece8e1` |
| **Accent** | Tan `#b8956a` |
| **Logo** | Letterspaced **B V L L Y** |
| **Display type** | UnifrakturMaguntia (gothic) |
| **Photos** | On-body lookbook in beige, electric blue, black, and army green |

---

## Content they will provide

| Item | Client provides? | Due date |
|------|-------------------|----------|
| Logo | Later | After this draft |
| More lookbook | Yes | After this draft |
| Final prices / colorways | Confirm against this draft | After this draft |

---

## Technical & launch

| Field | Answer |
|-------|--------|
| **Domain name** | TBD |
| **Hosting** | GitHub Pages (same path as Valley Notary / Vincere HTML kit) |
| **Contact form email** | hello@bvlly.com until replaced |
| **Kit chosen** | HTML |

**Notes:** Rough draft. No live checkout. Waitlist via FormSubmit.
